define(
    ...
    function (app) {
        app.controller('biddingMarketingStart_controller', [
            ...
            function ($scope, $rootScope, $http, $state, $stateParams, uploader, wfOperate, viewData, $timeout, wfWaiting, sogModal, sogWfControlOperationType, seagull2Url, ValidateHelper, sogValidator, sogOguType, $filter, errorDialog, configure, $window, $injector) {
                ...
                $scope.isMac = false;
                $scope.firstDate = $filter('date')($scope.viewModel.purchaseOfMarketing.lastReplyDeadline, 'yyyy-MM-dd HH:mm:00.000');
                ...
                $scope.editpurchaseDatearrange = function () {
                    wfWaiting.show();
                    $http.get(seagull2Url.getPlatformUrl("/BiddingMarketing/Update?resourceID=" + $scope.viewModel.resourceID + "&isAdjustBiddingDataArrange=true"))
                        .success(function () {
                            $scope.viewModel.purchaseOfMarketing.isAdjustBiddingDataArrange = true;
                            wfWaiting.hide();
                            var t_link = "default.htm?processDescKey=ReimbursementV2_ModifyPurchaseTime&parentResourceID=" + $scope.viewModel.resourceID + "&purchaseActivityID=" + viewData.currentActivityId + "&urlPart=biddingMarketingStart" + "#/modifyPurchaseTime/";
                ...
                $scope.changeIsAbandonBidding = function () {
                    var dataModel = { ResourceID: $scope.viewModel.resourceID, IsAbandonBidding: !$scope.viewModel.purchaseOfMarketing.isAbandonBidding };
