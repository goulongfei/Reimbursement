define(
    ...
    function (app) {
        app.controller('biddingBusinessOperationsTechnologyClarigy_controller', 
            function ($scope, viewData, wfWaiting, sogModal, ValidateHelper, sogValidator, rcTools, sogWfControlOperationType, wfOperate) {
                ...
                $scope.settings = {
                    ...
                    leftNavListOpts: {
                        ...
                        scene: "BiddingBusinessOperationsTechnologyClarigy",
                ...
                $scope.refreshProcess = function () {
                    var param = {
                        ...
                        TechnologyMainUserCode: $scope.viewModel.purchaseOfBusinessOperations.technologyMainUserCode,
                    };
                    if (param) {
                        wfOperate.refreshProcess('/BiddingBusinessOperationsTechnologyClarigyWf', $scope.currentActivityId, null, param, true);
                    }
                };
                ...
                var validData = function () {
                    ...
                    // 技术清标文件
                    var technologyClarigyFileUploaded = false;
                    if ($scope.viewModel.technologyClarigyFile && angular.isArray($scope.viewModel.technologyClarigyFile.clientFileInformationList)) {
                        for (var i = 0; i < $scope.viewModel.technologyClarigyFile.clientFileInformationList.length; i++) {
                            var item = $scope.viewModel.technologyClarigyFile.clientFileInformationList[i];
                            if (item.uploaded === true && item.isDeleted !== true) {
                                technologyClarigyFileUploaded = true;
                            }
                        }
                    }
                    if (technologyClarigyFileUploaded === false) {
                        modelStateDictionary.addModelError('技术清标文件', '请上传！');
                    }
