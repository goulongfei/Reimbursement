define(
    ...
    function (app) {
        app.controller('biddingImplementEvaluateSummary_controller', [
            '$scope', 'viewData',
            ...
            function ($scope, viewData,
                ...
                $scope.settings = {
                    ...
                    leftNavListOpts: {
                        ...
                        scene: "BiddingImplementEvaluateSummary",
                ...
                $scope.baseInfo = {
                    ...
                    setIsGrantDiscountEnquiry: function (val) {
                        $scope.viewModel.purchaseOfImplement.isGrantDiscountEnquiry = val;
                        if (val === true) {
                    ...
                    cancelBidding: function () {
                        $scope.viewModel.purchaseOfImplement.isGrantDiscountEnquiry = false;
                ...
                var validData = function () {
                    ...
                    if (!$scope.viewModel.isInvalidTender && !$scope.viewModel.purchaseOfImplement.isAbandonBidding) {
                        if ($scope.viewModel.purchaseOfImplement.isGrantDiscountEnquiry == null) {
                            modelStateDictionary.addModelError('是否发放让利询价', '不能为空！');
                        }
                        // 需发放标段 
                        if ($scope.viewModel.purchaseOfImplement.isGrantDiscountEnquiry) {
                            if (!$scope.viewModel.purchaseOfImplement.discountDeadline) {
                ...
                $scope.refreshProcess = function () {
                    var param = {};
                    if ($scope.viewModel.purchaseOfImplement.isAbandonBidding !== true) {
                        param.IsAbandonBidding = false;
                        param.IsGrantDiscountEnquiry = $scope.viewModel.purchaseOfImplement.isGrantDiscountEnquiry;
                    }
                    else {
                        param.IsAbandonBidding = $scope.viewModel.purchaseOfImplement.isAbandonBidding;
                        param.IsGrantDiscountEnquiry = false;
                    }
                    if (param) {
                        wfOperate.refreshProcess('/BiddingImplementEvaluateSummaryWf', $scope.currentActivityId, null, param, true);
                    }
                };
