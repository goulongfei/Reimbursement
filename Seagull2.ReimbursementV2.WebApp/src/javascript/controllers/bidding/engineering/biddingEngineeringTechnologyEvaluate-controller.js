define(
    ...
    function (app) {
        app.controller('biddingEngineeringTechnologyEvaluate_controller', [
            '$scope', 'viewData',
            ...
            function ($scope, viewData,
                ...
                $scope.settings = {
                    ...
                    leftNavListOpts: {
                        ...
                        scene: "BiddingEngineeringTechnologyEvaluate",
                ...
                var validData = function () {
                    ...
                    if ($scope.settings.supplierScopeOpts.model.industryDomainWithLabel
                        ...
                        for (var i = 0; i < $scope.viewModel.replySupplierScopeList.length; i++) {
                            ...
                            // 成本竞争力
                            if (item.labelLC90119 && !item.labelLC90119OptionValue) {
                                modelStateDictionary.addModelError(item.supplierName + '-' + item.labelLC90119.title, '请选择！');
                            }
                            // 与考察阶段项目经理是否一致	
                            ...
                            if (item.labelLC90114) {
                                ...
                                for (var j = 0; j < item.labelLC90114.options.length; j++) {
                                    ...
                                    if (op.selected) {
                                        ...
                                        if (op.optionValue === "OP90114014") {
                                            isOP90114014 = true;
                                            op90114014 = op;
                                            op.otherText = item.otherTextOP90114014;
                                        }
                                    }
                                }
                                ...
                                if (isOP90114014) {
                                    if (item.otherTextOP90114014 == undefined || item.otherTextOP90114014 == null || item.otherTextOP90114014 == '') {
                                        modelStateDictionary.addModelError(item.supplierName + '-' + item.labelLC90114.title + '-其他', '不能为空！');
                                    }
                                    else if (item.otherTextOP90114014.length < op90114014.otherTextFloor || item.otherTextOP90114014.length > op90114014.otherTextCeiling) {
                                        modelStateDictionary.addModelError(item.supplierName + '-' + item.labelLC90114.title + '-其他', '字符个数必须在' + op90114014.otherTextFloor + '~' + op90114014.otherTextCeiling + '之间！');
                                    }
                                }
                            }
                            if (isOP90114002) {
                        ...
                        if (angular.isArray($scope.viewModel.replySupplierScopeList)) {
                            for (var i = 0; i < $scope.viewModel.replySupplierScopeList.length; i++) {
                                ...
                                if (item.labelLC90114) {
                                    ...
                                    if (isOP90114014) {
                                        if (item.otherTextOP90114014 && (item.otherTextOP90114014.length < op90114014.otherTextFloor || item.otherTextOP90114014.length > op90114014.otherTextCeiling)) {
                                            modelStateDictionary.addModelError(item.supplierName + '-' + item.labelLC90114.title + '-其他', '字符个数必须在' + op90114014.otherTextFloor + '~' + op90114014.otherTextCeiling + '之间！');
                                        }
                                    }
                                }
