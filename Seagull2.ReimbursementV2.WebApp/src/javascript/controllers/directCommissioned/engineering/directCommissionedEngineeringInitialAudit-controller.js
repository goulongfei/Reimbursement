define(
    ...
    function (app) {
        app.controller('directCommissionedEngineeringInitialAudit_controller', [
            ...
            function ($scope, viewData, $rootScope, wfOperate, $http, seagull2Url,
                ...
                $scope.baseInfo = {
                    ...
                    firstTrialAmountChange: function () {
                        var total = rcTools.toFixedNum($scope.viewModel.purchaseOfEngineering.firstTrialAgreedPartAmount, 2) +
                            rcTools.toFixedNum($scope.viewModel.purchaseOfEngineering.firstTrialDisputePartAmount, 2);
                        ...
                        $scope.viewModel.purchaseOfEngineering.firstTrialCostTargetDeviationAmount = rcTools.toFixedNum(deviation, 2);
                        $scope.viewModel.purchaseOfEngineering.firstTrialCostTargetDeviationRate
                            = costTarget === 0 ? 0 : rcTools.toFixedNum((deviation / costTarget) * 100, 2);

                        $scope.viewModel.purchaseOfEngineering.firstTrialAmount = total;
                    ...
                    setIsMarketingExhibition: function (value) {
                        $scope.viewModel.purchaseOfEngineering.isMarketingExhibition = value;
                    },
                };
                ...
                var validData = function () {
                    ...
                    var modelStateDictionary = ValidateHelper.validateData($scope.viewModel, [
                        { key: '初审一致部分(元)', attributeName: 'purchaseOfEngineering.firstTrialAgreedPartAmount', validator: new RequiredValidator('不能为空！') }, 
                        { key: '成本目标偏差说明', attributeName: 'purchaseOfEngineering.firstTrialCostTargetDeviationExplain', validator: new RequiredValidator('不能为空！') },
                        { key: '本目标偏差说明', attributeName: 'purchaseOfEngineering.firstTrialCostTargetDeviationExplain', validator: new stringMaxLengthValidator(4000, "不能大于4000个字符!") },
                        { key: '初审单位', attributeName: 'purchaseOfEngineering.firstTrialUnitName', validator: new RequiredValidator('不能为空！') },
                        { key: '初审单位', attributeName: 'purchaseOfEngineering.firstTrialUnitName', validator: new stringMaxLengthValidator(100, "不能大于100个字符!") },
                        { key: '初审附件盖章', attributeName: 'purchaseOfEngineering.firstTrialIsNeedSeal', validator: new RequiredValidator('请选择！') }
                    ]);
                    // 预算初审报告
                ...
                $scope.refreshProcess = function () {
                    var param = {
                        FirstTrialAmount: $scope.viewModel.purchaseOfEngineering.firstTrialAmount,
                    ...
                    wfOperate.refreshProcess('/DirectCommissionedEngineeringInitialAuditWf', $scope.currentActivityId, null, param, true);
                };
