    ...
    [
        ...
        app.controller('compilingTender_controller',
            function ($injector, $scope, $rootScope, $http, $state, wfOperate, seagull2Url, wfWaiting, sogModal, $location, viewData,
                sogWfControlOperationType, sogOguType, sogValidator, ValidateHelper, $window, configure, supplierSelector, rcTools, supplierSelectorV2, locals, $timeout, configure, exception) {
                ...
                $scope.baseInfo = {
                    ...
                    depositAmountOnchange: function () {
                        if ($scope.viewModel.p_StrategyPurchasePlanCase.depositAmount == null || $scope.viewModel.p_StrategyPurchasePlanCase.depositAmount == undefined) {
                            $scope.viewModel.p_StrategyPurchasePlanCase.depositAmount = 0;
                        }
                    },
                    //招标人
                    ...
                    initConstructCenter: function () {
                        angular.forEach($scope.ConstructCenter, function (v) {
                            if (v.code == $scope.viewModel.p_StrategyPurchasePlanCase.biddingOrganizationCode) {
                                $scope.viewModel.p_StrategyPurchasePlanCase.biddingOrganizationName = v.cnName;
                                $scope.refreshProcess(true);
                            }
                        });
                    },
                    //选择合作期限
                ...
                $scope.$watch('viewModel.biddingOrganizationMainUser', function (newValue, oldValue) {
                ...
                //刷新流程
                $scope.refreshProcess = function (isBiddingOrganizationMainUser) {
                    ...
                    wfOperate.refreshProcess('/CompilingTenderWf', $scope.currentActivityId, null, {
                        biddingOrganizationMainUserID: $scope.viewModel.biddingOrganizationMainUser == null ? "" : $scope.viewModel.biddingOrganizationMainUser.id,
                        strategyPurchasePlanPersonIDList: strategyPurchasePlanPersonIDList,
                        isBiddingOrganizationMainUser: isBiddingOrganizationMainUser,
                        buildCenterCode: $scope.viewModel.p_StrategyPurchasePlanCase.biddingOrganizationCode
                    }).success(function (d) {
                ...
                var validData = function () {
                    ...
                    if (!$scope.viewModel.p_StrategyPurchasePlanCase.biddingOrganizationCode) modelStateDictionary.addModelError("采购信息", "招标组织不能为空");
                    ...
                    if (!$scope.viewModel.biddingOrganizationMainUser) modelStateDictionary.addModelError("采购信息", "招标组织集采对接人不能为空");
                    ...
                    if ($scope.viewModel.p_StrategyPurchasePlanCase.isPaymentedDepositAmount == null) modelStateDictionary.addModelError("采购信息", "是否已经缴纳不能为空");
                    if (!$scope.viewModel.p_StrategyPurchasePlanCase.perBiddingUnitCount || $scope.viewModel.p_StrategyPurchasePlanCase.perBiddingUnitCount <= 0) modelStateDictionary.addModelError("采购信息", "拟中标单位数量不能为0");
                    if (!$scope.viewModel.p_StrategyPurchasePlanCase.enterStandardExplain) modelStateDictionary.addModelError("采购信息", "入围标准（资质、业绩等）不能为空");
                    ...
                    angular.forEach($scope.viewModel.p_SupplierScopePEMus, function (item, i) {
                        if (item.lastYearAnnualValue == null || item.lastYearAnnualValue == 0) modelStateDictionary.addModelError("入围供应商", "第" + (i + 1) + "行去年总产值不能为0");
                        if (!item.performanceSituation) modelStateDictionary.addModelError("入围供应商", "第" + (i + 1) + "行业绩情况不能为空");
                    });
                    ...
                    if ($scope.viewModel.minutesMeetingFile == null || $scope.viewModel.minutesMeetingFile.length == 0) {
                        modelStateDictionary.addModelError("附件", "启动会会议纪要不能为空");
                    } else {
                        var count = 0;
                        angular.forEach($scope.viewModel.minutesMeetingFile, function (file) {
                            if (!file.isDeleted) {
                                count++;
                            }
                        });
