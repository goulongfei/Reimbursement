define([
    ...
    app.controller('startChargeApplication_controller', [
        '$scope', 'viewData', 'sogModal', 'sogWfControlOperationType', 'configure', '$window','$http','seagull2Url',
        function ($scope, viewData, sogModal, sogWfControlOperationType, configure, $window, $http, seagull2Url) {
                ...
                angular.forEach($scope.viewModel.repaymentInfoList, function (item) {
                    if (item!=null) {
                        ...
                        if ($scope.viewModel.purchaseOfNoContract.prepaymentRefundAmount != 0) {
                            $scope.repaymentInfoTotal.shouldReturnedTotal += $scope.viewModel.purchaseOfNoContract.prepaymentRefundAmount;
                        }
                    }
                });
                $scope.repaymentInfoTotal.actualRefundScaleTotal = (($scope.repaymentInfoTotal.amountReturnedTotal * 1.0) / $scope.repaymentInfoTotal.shouldReturnedTotal) * 100;
            }
            // 设置
            $scope.baseInfo = {
                // 无合同采购申请报告
