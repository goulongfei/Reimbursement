define([
    ...
    'contractAgreementExtend',
        ...
        app.controller('lookingUpNoContentView_controller', function ($scope, $http, sogModal, $location, seagull2Url, wfWaiting) {
            if (!$location.search().resourceID) {
                sogModal.openAlertDialog('提示', '地址中不存在resourceID!');
                return;
            } 
            wfWaiting.show();
            $http.get(seagull2Url.getPlatformUrl('/NoContractDataMigration/LookingUpNoContractViewData?resourceID=' + $location.search().resourceID), {
                cache: false
            }).success(function (response) {
                wfWaiting.hide();
                angular.extend($scope.viewModel, response);
                /////////////////////////初始化////////////////////////
                //流程标题
                ...
                wfWaiting.hide();
                sogModal.openAlertDialog("提示", data);
            });
