define([
    ...
    app.controller('purchaseSchemefileInfoProvided_controller',
        function ($scope, uploader, wfWaiting, sogModal, seagull2Url, viewData, ValidateHelper, sogWfControlOperationType, $http) {
            ...
            angular.forEach($scope.viewModel.materialFeedbackInfoList, function (item) {
                if (item.feedbackProcessID == $scope.viewModel.activityCode) {
                    $scope.opinionInfo = item;
                }
            });
            ...
            $scope.collectData = function (e, defer) {
                if (e.operationType === sogWfControlOperationType.MoveTo) {
                    if ($scope.viewModel.specialtyOpinionTyoeCode == null || $scope.viewModel.specialtyOpinionTyoeCode == undefined) {
                        sogModal.openAlertDialog('意见栏', '请选择同意或者不同意！');
                        return;
                    }
                }
